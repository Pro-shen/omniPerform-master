...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = 'omniperform' 
     AND TABLE_NAME = 'guide_performance' 
     AND COLUMN_NAME = 'cai_score') = 0,
    'ALTER TABLE guide_performance ADD COLUMN cai_score DECIMAL(5,4) DEFAULT 0 COMMENT "CAI指数"',
    'SELECT "cai_score字段已存在" as message'
));
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = 'omniperform' 
     AND TABLE_NAME = 'guide_performance' 
     AND COLUMN_NAME = 'rmv_score') = 0,
    'ALTER TABLE guide_performance ADD COLUMN rmv_score DECIMAL(5,4) DEFAULT 0 COMMENT "RMV指数"',
    'SELECT "rmv_score字段已存在" as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 检查并添加matrix_position字段
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = 'omniperform' 
     AND TABLE_NAME = 'guide_performance' 
     AND COLUMN_NAME = 'matrix_position') = 0,
    'ALTER TABLE guide_performance ADD COLUMN matrix_position VARCHAR(10) COMMENT "九宫格位置(如:3-3)"',
    'SELECT "matrix_position字段已存在" as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = 'omniperform' 
     AND TABLE_NAME = 'guide_performance' 
     AND COLUMN_NAME = 'matrix_type') = 0,
    'ALTER TABLE guide_performance ADD COLUMN matrix_type VARCHAR(50) COMMENT "九宫格类型"',
    'SELECT "matrix_type字段已存在" as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = 'omniperform' 
     AND TABLE_NAME = 'guide_performance' 
     AND COLUMN_NAME = 'trend') = 0,
    'ALTER TABLE guide_performance ADD COLUMN trend VARCHAR(20) COMMENT "趋势(提升/持平/下降)"',
    'SELECT "trend字段已存在" as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = 'omniperform' 
     AND TABLE_NAME = 'guide_performance' 
     AND COLUMN_NAME = 'new_members') = 0,
    'ALTER TABLE guide_performance ADD COLUMN new_members INT DEFAULT 0 COMMENT "新增会员数"',
    'SELECT "new_members字段已存在" as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
UPDATE guide_performance SET 
    ...
    trend = CASE 
        ...
        ELSE '下降'
    END
WHERE cai_score IS NULL OR cai_score = 0;
...
UPDATE guide_performance SET 
    ...
    matrix_type = CASE 
        ...
        ELSE '新手上路'
    END
WHERE matrix_position IS NULL OR matrix_position = '';
...
SELECT 
    COUNT(DISTINCT gi.id) as total_guides,
    COUNT(DISTINCT CASE WHEN gi.status = '在职' THEN gi.id END) as active_guides,
    ROUND(AVG(gp.performance_score), 1) as avg_performance_score,
    COUNT(DISTINCT CASE WHEN gp.matrix_position IN ('3-3', '3-2', '2-3') THEN gi.id END) as top_performer_count,
    ROUND(AVG(gp.cai_score), 3) as avg_cai,
    ROUND(AVG(gp.rmv_score), 3) as avg_rmv
FROM guide_info gi
LEFT JOIN guide_performance gp ON gi.guide_code = CONCAT('G', LPAD(gi.id, 8, '0')) 
    AND gp.period = DATE_FORMAT(NOW(), '%Y-%m')
WHERE gi.status = '在职';
...
CREATE INDEX IF NOT EXISTS idx_cai_rmv ON guide_performance(cai_score, rmv_score);
CREATE INDEX IF NOT EXISTS idx_matrix_position ON guide_performance(matrix_position);
CREATE INDEX IF NOT EXISTS idx_period_region ON guide_performance(period);
